...
public class TextStreamExtensionsTests
{
    ...
    public async Task ReadToIdleAsync_Array_Full()
    {
        ...
        // テスト用データ
        var dummy = Random.Shared.GetItems<char>("abcdefghijklmopqrstuvwxyzABCDEFGHIJKLMOPQRSTUVWXYZ", 5);
        var pad = '!';
        var buffer = new char[32].FillBy(pad);

        // テスト用パイプからの読み取りタスクを開始
        using var output = new StreamReader(pipe.Reader.AsStream());
        var idleTime = 3000;
        var readTask = output.ReadToIdleAsync(buffer, idleTime);

        // パイプに書き込み
        using var input = new StreamWriter(pipe.Writer.AsStream());
