...
public static class ThisSource
{
    ...
    public static FileInfo GetFile([CallerFilePath] string path = "") => new FileInfo(path);

    /// <summary>呼び出し元ソースファイルからの相対パスで示されるファイル情報を取得する。</summary>
    /// <param name="relativePath">相対パス</param>
    /// <param name="path">この引数は省略する必要がある。</param>
    /// <returns>相対ファイル情報</returns>
    public static FileInfo GetRelativeFile(string relativePath, [CallerFilePath] string path = "") => new FileInfo(Path.Combine(Path.GetDirectoryName(path)!, relativePath));

    /// <summary>呼び出し元ソースファイルからの相対パスで示されるディレクトリ情報を取得する。</summary>
    /// <param name="relativePath">相対パス</param>
    /// <param name="path">この引数は省略する必要がある。</param>
    /// <returns>相対ディレクトリ情報</returns>
    public static DirectoryInfo GetRelativeDirectory(string relativePath, [CallerFilePath] string path = "") => new DirectoryInfo(Path.Combine(Path.GetDirectoryName(path)!, relativePath));
