...
public static class TextStreamExtensions
{
    ...
    public static async ValueTask<int> ReadToIdleAsync(this TextReader self, Memory<char> buffer, int idle, CancellationToken cancelToken = default)
    {
    ...
    /// <param name="cancelToken">キャンセルトークン</param>
    /// <returns>読み取ったバイト数を得るタスク</returns>
    public static async ValueTask<int> ReadToIdleAsync(this TextReader self, IBufferWriter<char> writer, int idle, CancellationToken cancelToken = default)
    {
        if (idle < 0) throw new ArgumentException($"Invalid {nameof(idle)} time");
